 ...
 * 데이터를 처리하는 함수가 action 함수이다.
 * 이 함수를 react 가 호출할 때 2개의 매개변수 값을 전달하는데 request, params 이다.
...
export const updateAction = async ({ request, params }) => {
  ...
  /**  DB 로부터 SELECT 한 bucket 에 input 으로 입력한 데이터를 update 하여 새로운 bucket 생성하기 */
...
const BucketUpdate = () => {
  ...
  return (
    <Form className={css.bucket_form} method="POST">
      <article className={css.first}>
        <input
          type="file"
          accept="image/*"
          onChange={(e) => {
            onFileChange(e, bucket);
          }}
          ref={imgRef}
        />
        <div>
