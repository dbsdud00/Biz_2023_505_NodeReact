/**
 * web browser 의 indexDB 를 핸들링 하기 위한 react 3rd party LIB
 */
import localforage from "localforage";
import uuid from "react-uuid";
import moment from "moment";
/**
 * 코딩에서 매직 리터럴(매직 스트링, 매직넘버) 금지
...
export const newBucket = async () => {
  ...
  /** DB 에서 데이터를 가져왔는데 데이터가 하나도 없으면 (null) 빈배열([]) 로 초기화 해라 */
  const bucketList = (await getBucketList()) || [];
  /**
...
/**  browser 의 indexedDB 에 BUCKET_LIST 이름으로 데이터 저장하기 */
